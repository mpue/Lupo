...
namespace stk {
	...
	BlitSquare::BlitSquare(StkFloat frequency)
	{
		...
		nHarmonics_ = 0;
		pulseWidth_ = 0.5;  // Default to 50% duty cycle (standard square wave)
		lastPulseOutput_ = 0.0;  // Initialize pulse filter state
	...
	void BlitSquare::reset()
	{
		...
		dcbState_ = 0.0;
		lastBlitOutput_ = 0;
		lastPulseOutput_ = 0.0;  // Reset pulse filter state
	}
	...
	void BlitSquare::setPulseWidth(StkFloat width)
	{
